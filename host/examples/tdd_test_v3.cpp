...
void transmit_worker(
  ...
  uhd::time_spec_t timespec_tx_tdd = uhd::time_spec_t(0,tdd_tx_samps,rate);
      ...
      if(verbose) std::cout << boost::format(" timestamp init %ld") % timespec_tx_tdd.to_ticks(rate) << std::endl;
    ...
    while(not stop_signal_called){
       ...
       while(count_write == 0) {
       //printf("\nentering cond wait loop\n");
       pthread_cond_wait(&cond,&lock); // this unlocks mutex_rxtx while waiting and then locks it again
       }
     
       slot = tx_packet_count%20; 
       if (tx_packet_count != 0 ) {
           if(slot == 0) flag = 2; 
           if(slot >0 && slot <9) flag = 1;
           if(slot == 9) flag = 3; 
           if(slot >9 && slot <20) flag = 0;
           timestamp_tx = timestamp_tx + tdd_tx_samps;
       }
       ...
       if(tx_packet_count !=0 && flag == 1) { // flag == 1 continuous tx packets
         ...
         tx_md.time_spec = uhd::time_spec_t::from_ticks(timestamp_tx, rate);
       }
      ...
      if(flag == 3) usrp->clear_command_time();
      gpio789 = (gpio789+1)&7; 
      ...
      num_tx_samps = tx_stream->send(&tx_buff.front(),tdd_tx_samps, tx_md);
  
      if(num_tx_samps != tdd_tx_samps){
        printf("ERROR_NUMBER_OF_SAMPLES");
      }

      if(verbose) std::cout << boost::format("TX packet: %u samples, %u full secs, %f frac secs %ld timestamp %d slot %d flag %d count_write") % num_tx_samps % tx_md.time_spec.get_full_secs() % tx_md.time_spec.get_frac_secs() % tx_md.time_spec.to_ticks(rate) % slot % flag % count_write << std::endl;
      }
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
   ...
   //detect which channels to use
    ...
    timestamp_rx = stream_cmd.time_spec.get_tick_count(rate);

    //meta-data will be filled in by recv()
    uhd::rx_metadata_t rx_md;

    //allocate buffer to receive with samples
    std::vector<std::complex<float> > rx_buff(15360);
    ...
    double timeout = seconds_in_future ; //timeout (delay before receive + padding)
    ...
    do {
	  ...
	  num_rx_samps = rx_stream->recv(rx_buffs, tdd_rx_samps, rx_md,timeout);

	  //use default timeout for subsequent packets
	  timeout = 0.1;
	  ...
	  if (rx_md.error_code != uhd::rx_metadata_t::ERROR_CODE_NONE){
        ...
        pthread_mutex_lock(&lock);
        timestamp_rx = rx_md.time_spec.to_ticks(rate);
        
        if (recv_packet==0){
          timestamp_tx = timestamp_rx+6*rate*0.000125;
        }
	...
	if(verbose) std::cout << boost::format("RX packet: %u samples, %u full secs, %f frac secs %ld timestamp %d count_write") % num_rx_samps % rx_md.time_spec.get_full_secs() % rx_md.time_spec.get_frac_secs()% rx_md.time_spec.to_ticks(rate) % count_write << std::endl;
